...
//            loadExistingCertificates();
    ...
    public void loadExistingCertificates() throws NoSuchProviderException, OperatorCreationException, CertificateException, KeyStoreException, IOException, NoSuchAlgorithmException {
        ...
        for (com.example.model.Certificate c : allCertificates) {
            ...
            String certificateFilename = c.getSerialNumber() + ".cer";
            saveCertificateToFile(generatedCertificate, certificateFilename);

            // Save the private key to a file
            String privateKeyFilename = c.getSerialNumber() + ".pem";
            savePrivateKeyToFile(privateKey, privateKeyFilename);
        }
    }

    private void saveCertificateToFile(X509Certificate certificate, String filename) throws IOException, CertificateEncodingException {
        FileOutputStream fos = new FileOutputStream("C://Users//Svetozar//Desktop//DRUGI SEMESTAR//Informaciona bezbednost//Projekat//BackendTim22IB//src//main//java//com//example//certificates//certificates/" + filename);
        fos.write(certificate.getEncoded());
        fos.close();
    }

//    private void savePrivateKeyToFile(PrivateKey privateKey, String filename) throws IOException {
...
//
//        // Save the private key to a file
//        FileWriter fw = new FileWriter("path/to/privatekeys/" + filename);
//        fw.write(sw.toString());
//        fw.close();
//    }

    private void savePrivateKeyToFile(PrivateKey privateKey, String filename) throws IOException {
        // Convert the private key to PKCS8EncodedKeySpec
        PKCS8EncodedKeySpec privateKeySpec = new PKCS8EncodedKeySpec(privateKey.getEncoded());

        // Save the private key to a file
        try (FileOutputStream outputStream = new FileOutputStream("C://Users//Svetozar//Desktop//DRUGI SEMESTAR//Informaciona bezbednost//Projekat//BackendTim22IB//src//main//java//com//example//certificates//privatekeys/" + filename)) {
